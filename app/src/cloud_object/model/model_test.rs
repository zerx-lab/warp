...
// Regression test for #311: deleting a locally-created object (a `SyncId::ClientId`,
// which is what every object gets now that cloud sync is gone) used to silently no-op
// because `delete_object_with_initiated_by` required `id.server_id()` to be `Some`.
#[test]
fn test_delete_local_object_removes_it() {
    App::test((), |mut app| async move {
        initialize_app(&mut app, Vec::new());
        ...
        ObjectStoreModel::handle(&app).read(&app, |model, _| {
            assert!(
                model
                    ...
                    .is_some(),
                "rule should exist after creation"
            );
        });
        ...
        ObjectStoreModel::handle(&app).read(&app, |model, _| {
            assert!(
                model
                    .get_object_of_type::<GenericStringObjectId, AIFactObjectModel>(&sync_id)
                    .is_none(),
                "rule should be gone after delete_object_by_user"
            );
        });
    });
}
